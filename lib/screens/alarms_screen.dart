...
class _AlarmsScreenState extends State<AlarmsScreen> {
  ...
  void _startAlarmChecker() {
    Timer.periodic(const Duration(seconds: 10), (_) => _checkAlarms());
  }
  ...
  void _checkAlarms() {
    ...
    for (final alarm in _alarms) {
      ...
      if (alarm.hour == now.hour && alarm.minute == now.minute) {
        ...
        if (alarm.days.isEmpty || alarm.days.contains(dayIndex)) {
          _fireAlarm(alarm);
          break;
        }
      }
  ...
  void _fireAlarm(Alarm alarm) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => AlarmFiringScreen(alarm: alarm, onDismiss: () {
          setState(() {});
          _loadStats();
        }),
  ...
  String _nextAlarmText() {
    ...
    if (best == null) return '';
    return 'próxima: ${best.timeStr} — ${best.label}';
  }
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            fontSize: 36,
                            fontWeight: FontWeight.w800,
                            color: alarm.enabled ? AppTheme.textColor : AppTheme.muted,
                            height: 1,
                            letterSpacing: -1,
                          ),
                        ),
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            _tag(diffLabels[alarm.difficulty] ?? 'Fácil',
                                diffColors[alarm.difficulty] ?? AppTheme.green),
                            if (alarm.snoozeCount > 0)
                              _tag('😴×${alarm.snoozeCount}', AppTheme.orange),
                          ],
                        ),
                        const SizedBox(height: 8),
                        // Botón probar
                        GestureDetector(
                          onTap: () => _fireAlarm(alarm),
                          child: Container(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('Sin alarmas todavía',
              style: TextStyle(fontSize: 14, color: AppTheme.muted, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          const Text('Pulsa + Nueva para crear una',
