...
class _AlarmFiringScreenState extends State<AlarmFiringScreen>
  ...
  void initState() {
    super.initState();
    _bellController = AnimationController(vsync: this, duration: const Duration(milliseconds: 400))
      ..repeat(reverse: true);
    ...
    if (ct == 'random') {
      const types = ['math','incognita','shake','typing','pattern','cultura'];
      ct = types[Random().nextInt(types.length)];
    }
    _challengeType = ct;
    _loadChallenge();
    _startVibration();
    // NO llamamos AudioService.play aquí — el AlarmService ya está sonando
  }
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (t) {
      if (!mounted) { t.cancel(); return; }
      setState(() => _timerLeft--);
      if (_timerLeft <= 0) { t.cancel(); _penalize(); }
    });
  }
  ...
  void _penalize() {
    ...
    Future.delayed(const Duration(milliseconds: 1300), () { if (mounted) _loadChallenge(); });
  }
  ...
  void _loadMath() {
    final rng = Random();
    final ops = _difficulty=='easy'?['+']:_difficulty=='medium'?['+','-']:['+','-','×'];
    final op  = ops[rng.nextInt(ops.length)];
    final mx  = _difficulty=='easy'?20:_difficulty=='medium'?60:120;
    int a,b,ans;
    if(op=='+'){a=rng.nextInt(mx)+2;b=rng.nextInt(mx)+2;ans=a+b;}
    else if(op=='-'){a=rng.nextInt(mx~/2)+mx~/2;b=rng.nextInt(a)+1;ans=a-b;}
    else{a=rng.nextInt(12)+2;b=rng.nextInt(12)+2;ans=a*b;}
    final sp=_difficulty=='easy'?10:_difficulty=='medium'?25:60;
    final wr=<int>{};
    while(wr.length<3){final w=ans+rng.nextInt(sp*2+1)-sp;if(w!=ans&&w>0)wr.add(w);}
    final opts=[...wr,ans]..shuffle();
    setState((){_equation='$a $op $b = ?';_correctAnswer=ans;_options=opts;});
  }

  void _loadInc() {
    final rng=Random();
    int x,a,b,ans; String eq;
    if(_difficulty=='easy'){x=rng.nextInt(20)+1;b=rng.nextInt(20)+1;ans=x;eq='X + $b = ${x+b}';}
    else{a=rng.nextInt(8)+2;x=rng.nextInt(20)+1;b=rng.nextInt(30)+1;ans=x;eq='${a}X + $b = ${a*x+b}';}
    final sp=15;
    final wr=<int>{};
    while(wr.length<3){final w=ans+rng.nextInt(sp*2+1)-sp;if(w!=ans&&w>0)wr.add(w);}
    final opts=[...wr,ans]..shuffle();
  ...
  void _loadShake(){_shakeTarget=_difficulty=='easy'?15:_difficulty=='medium'?25:40;setState((){_shakeCount=0;});}
  ...
  void _loadPattern(){
    ...
    _userPattern=[];_showingPattern=true;_patternMsg='Observa el patrón...';
    setState((){});
    _showPatternAnim(showMs);
  }

  void _showPatternAnim(int ms) async {
    for(int i=0;i<_pattern.length;i++){if(!mounted)return;setState((){});await Future.delayed(Duration(milliseconds:ms));}
    if(mounted)setState((){_showingPattern=false;_patternMsg='¡Repite el patrón!';});
  }

  void _loadCultura(){
    final bank=_culturaBank[_difficulty]??_culturaBank['easy']!;
    final q=bank[Random().nextInt(bank.length)];
    setState((){_question=q['q'] as String;_qOpts=List<String>.from(q['opts'] as List);_correctIdx=q['ans'] as int;_answered=false;});
  }
  ...
  void _advance(){
    ...
    _challengesPassed++;
    if(_challengesPassed>=_challengesTotal){_dismiss();}
    else{
      ...
      setState((){});
      Future.delayed(const Duration(milliseconds:600),(){if(mounted)_loadChallenge();});
    }
  }

  /// DETENER TODO y registrar en historial
  Future<void> _stopAll() async {
    _timer?.cancel();
    // Detener el AlarmService nativo (sonido real)
    await AlarmScheduler.stopAlarmService();
    // Detener AudioService por si acaso
  ...
  void dispose() {
    _timer?.cancel();
    // Seguridad: si se cierra la pantalla sin resolver, detener todo
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false, // ← evita cerrar con botón atrás sin resolver
      child: Scaffold(
            ...
            builder: (_,__) => Container(
              decoration: BoxDecoration(
                gradient: RadialGradient(
                  center: const Alignment(0, -0.4), radius: 1.2,
                  colors: [AppTheme.red.withOpacity(0.12 * _glowController.value), Colors.transparent],
                ),
              ),
            ),
          ),
          SafeArea(child: Column(children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                SizedBox(width: 24, child: Text('$_timerLeft',
              ...
              child: Container(
                ...
                child: Column(children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    Text(_challengeLabel(),
                        style: const TextStyle(fontSize: 9, color: AppTheme.orange,
                            letterSpacing: 3, fontWeight: FontWeight.w600)),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                      decoration: BoxDecoration(color: _diffColor().withOpacity(0.15), borderRadius: BorderRadius.circular(4)),
                      child: Text(_difficulty=='easy'?'FÁCIL':_difficulty=='medium'?'MEDIO':'DIFÍCIL',
                          style: TextStyle(fontSize: 8, color: _diffColor(), fontWeight: FontWeight.w600, letterSpacing: 1)),
                    ),
                  ]),
  ...
  Widget _buildChallenge() {
    switch (_challengeType) {
      case 'math': case 'incognita': return _buildMath();
      case 'shake':   return _buildShake();
      case 'pattern': return _buildPattern();
      case 'cultura': return _buildCultura();
      default:        return _buildMath();
    }
  }

  Widget _buildMath() => Column(children: [
    Container(
    ...
    if (_challengeType=='incognita') ...[
      ...
      Container(
        ...
        decoration: BoxDecoration(color: AppTheme.purple.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
        child: Text(_equation, textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: AppTheme.purple, letterSpacing: 2)),
      ),
    ],
    const SizedBox(height: 14),
    GridView.count(
      crossAxisCount: 2, crossAxisSpacing: 8, mainAxisSpacing: 8,
      shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), childAspectRatio: 2.2,
      children: _options.map((opt) {
        ...
        return ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: AppTheme.s2, foregroundColor: AppTheme.textColor,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            elevation: 0, side: BorderSide(color: Colors.white.withOpacity(0.06)),
          ),
  ...
  Widget _buildShake() {
    ...
    return Column(children: [
      ...
      RichText(text: TextSpan(children: [
        TextSpan(text: '$_shakeCount', style: const TextStyle(fontSize: 64, fontWeight: FontWeight.w800, color: AppTheme.red, height:1)),
        TextSpan(text: ' / $_shakeTarget', style: const TextStyle(fontSize: 32, fontWeight: FontWeight.w800, color: AppTheme.muted, height:1)),
      ])),
      const SizedBox(height: 14),
      ClipRRect(borderRadius: BorderRadius.circular(3),
        child: LinearProgressIndicator(value: pct.toDouble(), backgroundColor: AppTheme.s3,
            valueColor: const AlwaysStoppedAnimation(AppTheme.red), minHeight: 6)),
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [AppTheme.red, AppTheme.orange],
  ...
  void _tapPattern(int idx) {
    ...
    final exp = _pattern[_userPattern.length];
    _userPattern.add(idx);
    if (idx != exp) {
      HapticFeedback.heavyImpact();
      setState(() { _feedbackText='✗ Incorrecto'; _feedbackOk=false; });
      Future.delayed(const Duration(milliseconds:1500), () { if(mounted){_userPattern.clear();_loadPattern();} });
      return;
    }
    ...
    return Column(children: [
      Container(
        ...
        child: Text(_question,
            style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppTheme.textColor, height:1.4)),
      ),
      const SizedBox(height: 12),
      ...List.generate(_qOpts.length, (i) => Padding(
        ...
        child: GestureDetector(
          onTap: _answered ? null : () {
            ...
            if (i == _correctIdx) {
              ...
              HapticFeedback.heavyImpact();
              setState(() { _feedbackText='✗ Incorrecto'; _feedbackOk=false; });
              Future.delayed(const Duration(milliseconds:2500), () { if(mounted)_loadCultura(); });
            }
          },
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _answered && i==_correctIdx ? AppTheme.green.withOpacity(0.15) : AppTheme.s2,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: _answered && i==_correctIdx ? AppTheme.green : Colors.white.withOpacity(0.06)),
            ),
            child: Row(children: [
              Text('${labels[i]}.  ', style: TextStyle(fontSize: 11,
                  color: _answered && i==_correctIdx ? AppTheme.green : AppTheme.muted,
                  fontWeight: FontWeight.w700)),
              Expanded(child: Text(_qOpts[i], style: const TextStyle(fontSize: 13, color: AppTheme.textColor))),
            ]),
          ),
        ),
      )),
  ...
  String _challengeLabel() {
    const labels = {
      ...
      'shake':'📳 AGITAR','pattern':'🟦 PATRÓN VISUAL','cultura':'🔬 CULTURA GENERAL',
    };
